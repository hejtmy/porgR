...
twosamplettest <- function(table=tgtw_table,tested_students = c("Josefina","Andrea")){
  ...
  hist <- ggplot(t_table) + geom_histogram(aes(Time, fill=Student),bins=20) + 
    geom_vline(xintercept = t$estimate[1]) + 
    geom_vline(xintercept = t$estimate[2])
  print(hist)
  print(t)
}
...
onesamplettest <- function(table=tgtw_table,tested_value=10,tested_student = "Joesefina"){
  ...
  hist <- ggplot(t_table) + geom_histogram(aes(Time),bins=20) + 
    geom_vline(xintercept = t$estimate[1]) + 
    geom_vline(xintercept = tested_value,colour="red")
  print(hist)
  print(t)
}
...
## ANOVA
summary(aov(tgtw_table$Time ~ tgtw_table$Student))
TukeyHSD(aov(tgtw_table$Time ~ tgtw_table$Student))
